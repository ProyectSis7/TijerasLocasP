...
app = FastAPI(title="Tijeras Locas API")
...
row = await cur.fetchone()
if row["c"] == 0:
await db.execute("INSERT INTO services (name, description, price, duration_min) VALUES ('Corte de pelo', 'Corte clásico o moderno con acabado a navaja.', 20.00, 30)")
await db.execute("INSERT INTO services (name, description, price, duration_min) VALUES ('Barba', 'Perfilado, rebaje y ritual de toalla caliente.', 15.00, 20)")
await db.execute("INSERT INTO services (name, description, price, duration_min) VALUES ('Combo', 'Corte completo y arreglo de barba premium.', 30.00, 50)")
...
("Luis Torres", 3, 1, (today - timedelta(days=5)).isoformat(), "12:00", "confirmed"),
("Pedro Martínez", 1, 2, (today - timedelta(days=4)).isoformat(), "09:30", "confirmed"),
("Juan García", 2, 3, (today - timedelta(days=4)).isoformat(), "15:00", "confirmed"),
("Miguel Ruiz", 3, 1, (today - timedelta(days=3)).isoformat(), "11:00", "confirmed"),
("Andrés López", 1, 3, (today - timedelta(days=2)).isoformat(), "14:00", "confirmed"),
("Sofía Díaz", 2, 2, (today - timedelta(days=1)).isoformat(), "10:30", "confirmed"),
("Pablo Fernández", 1, 1, today.isoformat(), "17:30", "pending"),
]
for c in citas:
await db.execute(
"INSERT INTO appointments (client_name, barber_id, service_id, appointment_date, appointment_time, status) VALUES (?,?,?,?,?,?)",
c
)
...
async with db.execute("""
SELECT a.id, a.client_name, a.appointment_time, a.status,
b.name AS barber_name, s.name AS service_name, s.duration_min
FROM appointments a
JOIN barbers b ON b.id = a.barber_id
JOIN services s ON s.id = a.service_id
WHERE a.appointment_date = ? ORDER BY a.appointment_time
""", (today.isoformat(),)) as cur:
...
weekly_revenue = sum(b["total_revenue"] for b in barbers)

return {
"kpis": {
"weekly_revenue": weekly_revenue,
"total_today": len(today_apts),
"pending_today": sum(1 for a in today_apts if a["status"] == "pending"),
"gap_alerts": len(gaps),
"week_start": week_start.isoformat(),
"week_end": week_end.isoformat(),
"today": today.isoformat(),
},
"barber_ranking": barbers,
"service_ranking": services,
